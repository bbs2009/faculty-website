					...
					<form onSubmit={handleSubmit}>
						<div className={styles.contacts_form_first_row}>
							...
							<div className={styles.field}>
								...
								<input type="text" name='name' id='name' placeholder="Ім'я" onChange={onChangeHandler} maxlength="50"/>
							</div>
							<div className={styles.field}>
								<label htmlFor="email">Електронна адреса</label>
								{isError ? <span className={styles.error} >{emailError}</span> : ''}
								<input type="email" placeholder="Email" id='email' required={true} onChange={onChangeHandler} maxlength="100"/>
							</div>
						</div>
						<div className={styles.contacts_form_second_row}>
							{isError ? <span className={styles.error} >{messageError}</span> : ''}
							<label htmlFor="message">Повідомлення</label>
							<textarea name="message" id='message' placeholder="Повідомлення" onChange={onChangeHandler} maxlength="2000"></textarea>
						</div>
